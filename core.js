            ...
            try {
                ...
                switch (command.type) {
                    ...
                    case 'SWAP': {
                        ...
                        const NULL_REGEX = /(\.|\[)__NULL__\]?$/;

                        // --- 1. DELETE MODE ---
                        // Syntax: @.SWAP("path.to.item", "__DELETE__");
                        if (path1 === DELETE_KEYWORD || path2 === DELETE_KEYWORD) {
                            if (path1 === DELETE_KEYWORD && path2 === DELETE_KEYWORD) break; // no-op

                            const targetPath = path1 === DELETE_KEYWORD ? path2 : path1;
                            const itemVal = _.get(state.static, targetPath);
                            
                            // If item doesn't exist, we can't delete it
                            if (itemVal === undefined) break;

                            // Perform deletion
                            const pathArr = _.toPath(targetPath);
                            const key = pathArr[pathArr.length - 1];
                            const parentPath = pathArr.slice(0, -1).join('.');
                            const parent = parentPath === '' ? state.static : _.get(state.static, parentPath);
                            ...
                            if (Array.isArray(parent)) {
                                ...
                                _.unset(state.static, targetPath);
                            }
                            break; 
                        }

                        // --- 2. REBASE MODE (One-way Move) ---
                        // Syntax: @.SWAP("path.to.item", "new.parent[__NULL__]");
                        const isNull1 = NULL_REGEX.test(path1);
                        const isNull2 = NULL_REGEX.test(path2);

                        if (isNull1 || isNull2) {
                            if (isNull1 && isNull2) break; // no-op

                            // Identify Item (Source) and Base (Target)
                            const itemPath = isNull1 ? path2 : path1;
                            const rawTargetNullPath = isNull1 ? path1 : path2;

                            // Strip the suffix to get the container path
                            const basePath = rawTargetNullPath.replace(NULL_REGEX, '');
                            ...
                            if (Array.isArray(base)) {
                                ...
                                // Re-use original key name at new location
                                const newPath = basePath ? `${basePath}.${key}` : key;
                                _.set(state.static, newPath, itemVal);
                            }
                            break;
                        ...
                        // --- 3. STANDARD SWAP MODE ---
                        // Syntax: @.SWAP("path.to.a", "path.to.b");
                        const getInfo = (p) => {
                        ...
                        // Remove both items from their respective parents
                        // Special handling if they are in the same array to avoid index shifting issues
