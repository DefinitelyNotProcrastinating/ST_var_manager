...
#### 更新预告
- O(1)的找回实际上并不好。我们发现它存储的变量块**很大**，所以我正在开发一个O(N)时间O(N)空间的架构。即使是1000回复对于一个电脑来说也很快，比AI回复快多了，我们基本可以把它当成一个常数很高的O(1)。你们不会真的玩卡玩到10000回复吧？！
- 新架构依然会使用FSM，但现在使用基于delta / commit的变量管理。
- 将成为一个真正的plugin，但依然会部分依赖酒馆助手的功能。
- 将重组自定义函数功能。
- 将重组一些更多的功能。
- 将使用一个新的idea，将把变量持续的部分交给AI执行。这意味着我们在跟踪变量更新时，将利用酒馆内置的变量更新模式，使用SET 更新每一个delta。
